...
# SAM wird jetzt einfach über die normale pip-Installation geladen
...
def evaluate_mask(mask_pred: np.ndarray, mask_gt: np.ndarray, gt_box: List[float]) -> Dict[str, object]:
    ...
    return {
        "mask_area": pred_area,
        "dice_score": float(dice),  # <-- Hier wird es hinzugefügt
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="SAM model type. vit_b is the lightest option.",
    )
    parser.add_argument("--output_dir", type=str, default="./results", help="Directory to save results and plots.")
    parser.add_argument("--max_annotations", type=int, default=None, help="Maximum number of annotations to evaluate. If None, use all annotations.")
...
def main() -> None:
    ...
    if args.max_annotations is not None:
        ...
        print(f"Using all {len(annotations)} annotations (no limit set)")

    # SAM laden (kein Repository-Pfad mehr nötig)
    ...
    if len(ok_df) > 0:
        # DICE_SCORE wurde hier hinzugefügt!
        metric_cols = [
            "sam_score",
            "dice_score",  # <-- Neu
