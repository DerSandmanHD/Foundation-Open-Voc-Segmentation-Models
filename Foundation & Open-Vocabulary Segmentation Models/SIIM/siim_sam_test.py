...
# ============================================================
# 1. Pfade konfigurieren (BITTE PRÜFEN)
# ============================================================
...
MAX_IMAGES = 20  # Für den Testlauf

os.makedirs(OUT_DIR, exist_ok=True)

# ============================================================
# 2. Hilfsfunktionen (Metriken & Bounding Boxes)
# ============================================================
def mask_to_multiple_bboxes(mask_np):
    """Trennt die Maske in einzelne Krankheitsherde und berechnet Boxen."""
...
def get_bbox_from_mask(mask_np):
    """Holt eine einzige Box aus einer vorhergesagten Maske für den IoU-Vergleich."""
    ys, xs = np.where(mask_np > 0)
    if len(xs) == 0 or len(ys) == 0: return None
    return [int(xs.min()), int(ys.min()), int(xs.max()), int(ys.max())]

def dice_score(pred, gt):
    """Berechnet den Pixel-genauen Dice-Score."""
    pred, gt = pred > 0, gt > 0
...
def calculate_iou(box1, box2):
    """Berechnet den Box-IoU für die NIH-Vergleichbarkeit."""
...
def eval_metrics(pred_box, gt_box, iou_thresh=0.5):
    """Prüft, ob der IoU groß genug für einen True Positive (Recall) ist."""
...
def main():
    ...
    print(f"\nLade {len(valid_pairs)} Bilder für den finalen Benchmark...")

    print("Lade SAM 1 (Facebook)...")
    ...
    # Tracking-Listen für die Ergebnisse
    dice_sam1_list, dice_medsam_list = [], []
    iou_sam1_list, iou_medsam_list = [], []
    tp_sam1, tp_medsam = 0, 0
    total_boxes = 0

    for img_path, mask_path, filename in tqdm(valid_pairs, desc="Benchmark läuft"):
        image = Image.open(img_path).convert("RGB")
        gt_mask = np.array(Image.open(mask_path).convert("L")) > 0
        
        # Ground Truth Boxen berechnen
        gt_boxes = mask_to_multiple_bboxes(gt_mask)
        if not gt_boxes: continue

        total_boxes += len(gt_boxes)

        # Wenn du DINO testen willst, müsstest du hier deine DINO-Funktion aufrufen:
        # prompt_boxes = get_dino_boxes(image, "pneumothorax") 
        # Und dann unten 'prompt_boxes' anstatt 'gt_boxes' in die Schleife geben.
        ...
        # Pixel-Dice Metriken berechnen (Bild-Level)
        ...
        # ============================================================
        # Benchmark-Visualisierung (4 Spalten)
        # ============================================================
        ...
        axes[1].set_title("Ground Truth (Ärzte)")
        axes[1].axis("off")
    ...
    # ============================================================
    # Konsolen-Ausgabe der finalen Metriken
    # ============================================================
    if total_boxes > 0:
        ...
        print("\n" + "="*60)
        print("📊 FINALE BENCHMARK ERGEBNISSE (SIIM Pneumothorax)")
        print("="*60)
        print("🔹 MODELL: SAM 1 (Normal)")
        print(f"   Pixel-Level: Mittlerer Dice-Score = {np.mean(dice_sam1_list):.4f}")
        print(f"   Box-Level:   Mittlerer Box-IoU    = {np.mean(iou_sam1_list):.4f}" if iou_sam1_list else "   Box-Level:   Mittlerer Box-IoU    = N/A")
        print(f"   Box-Level:   Recall@50            = {recall_s1:.4f}")
        print("-" * 60)
        print("🔹 MODELL: MedSAM (Medizin)")
        print(f"   Pixel-Level: Mittlerer Dice-Score = {np.mean(dice_medsam_list):.4f}")
        print(f"   Box-Level:   Mittlerer Box-IoU    = {np.mean(iou_medsam_list):.4f}" if iou_medsam_list else "   Box-Level:   Mittlerer Box-IoU    = N/A")
